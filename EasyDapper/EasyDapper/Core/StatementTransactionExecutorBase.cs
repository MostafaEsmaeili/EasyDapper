...
namespace EasyDapper.Core
{
    public abstract class StatementTransactionExecutorBase : IStatementExecutor
    {
        ...
        public int ExecuteNonQuery(string sql)
        {
            ...
            sqlConnection.Open();
            using (var command = sqlConnection.CreateCommand(connectionProvider.GetDbTransaction))
            {
        ...
        public async Task<int> ExecuteNonQueryAsync(string sql)
        {
            ...
            int num;
            using (var command = connection.CreateCommand(connectionProvider.GetDbTransaction))
            {
                command.CommandTimeout = 300000;
                command.CommandType = CommandType.Text;
                command.CommandText = sql;
            ...
            params ParameterDefinition[] parametersDefinitions)
        {
            ...
            using (var command = sqlConnection.CreateCommand(null))
            {
                ...
                if (parametersDefinitions
                    .Where(
                        m => m.Direction > ParameterDirection.Input)
                    .Count() > 0)
                {
