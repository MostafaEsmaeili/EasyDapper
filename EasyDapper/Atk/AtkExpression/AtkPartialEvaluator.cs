...
namespace Atk.AtkExpression
{
      ...
      private HashSet<Expression> candidates;
            ...
            private Expression Evaluate(Expression e)
            {
                ...
                if (e.NodeType == ExpressionType.Convert &&
                    AtkTypeHelper.GetNonNullableType(((UnaryExpression) e).Operand.Type) ==
                    AtkTypeHelper.GetNonNullableType(type))
                    e = ((UnaryExpression) e).Operand;
                if (e.NodeType == ExpressionType.Constant)
                {
                    var constantExpression = (ConstantExpression) e;
                    if (e.Type != type && AtkTypeHelper.GetNonNullableType(e.Type) ==
                        AtkTypeHelper.GetNonNullableType(type))
                ...
                return Expression.Constant(
                    Expression.Lambda<Func<object>>(e, Array.Empty<ParameterExpression>()).Compile()(), type);
            }
        }
    ...
    private class Nominator : ExpressionVisitor
    {
      ...
      {
        AtkPartialEvaluator.Nominator nominator = new AtkPartialEvaluator.Nominator(fnCanBeEvaluated);
        ...
        return nominator.candidates;
            ...
            protected override Expression Visit(Expression expression)
            {
                if (expression != null)
                {
                    var cannotBeEvaluated = this.cannotBeEvaluated;
